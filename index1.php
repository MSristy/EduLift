...
// --- Handle Campaign Creation Form Submission (from start-campaign.php) ---
$message = '';
...
// Fetch all fundraisers from the database that have NOT reached their goal, ordered by creation date (newest first)
$sql = "SELECT id, title, goal, amount_raised, image_path FROM fundraisers WHERE amount_raised < goal ORDER BY created_at DESC";
$result = $conn->query($sql);

$fundraisers = [];
// Check if there are results and fetch them into an array
...
<head>
    ...
    <title>Fundraisers Home Page</title>
        ...
        <section class="fundraiser-category">
            ...
            <div class="fundraiser-grid">
                <?php if (count($fundraisers) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p>No active fundraisers found yet. Start a campaign!</p>
                <?php endif; ?>
            </div>
        </section>
